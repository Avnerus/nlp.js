...
async function createProfessor(req, res) {
  try {
    ...
    // Insert professor into database with corpus as text (JSON string)
    // The integer ID is auto-generated by the database
    await sql`
      INSERT INTO professors (name, field, image, corpus, created_at)
      VALUES (${name}, ${field}, ${imageUrl}, ${JSON.stringify(corpus)}, ${new Date()})
    `;
    
    // Fetch the newly created professor to get the integer ID
    const professor = await sql`SELECT * FROM professors WHERE name = ${name} AND created_at = ${new Date()}`;
    
    const newProfessor = {
      id: professor[0].id,  // Integer ID from database
      name: professor[0].name,
      field: professor[0].field,
      image: professor[0].image,
      corpus: JSON.parse(professor[0].corpus),  // Parse back to object
      createdAt: professor[0].created_at
    };
